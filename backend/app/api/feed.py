...
async def get_topic_papers(
    ...
    weekly_digest = ""
    if page == 0:  # only on first page load
        digest_key = f"TOPIC_DIGEST_{topic}_{current_week}"
        from app.core.database import get_system_config, set_system_config
        weekly_digest = await get_system_config(digest_key) or ""
        if not weekly_digest and page_papers:
            try:
                from app.services.ai_service import AIValidationService
                from app.core.config import settings
                ...
                summaries = [
                    p.get("ai_lay_summary") or p.get("ai_summary") or p.get("abstract", "")[:200]
                    for p in page_papers[:8]
                    if p.get("ai_lay_summary") or p.get("ai_summary") or p.get("abstract")
                ]
                digest = await _ai.generate_topic_weekly_digest(topic_label_str, summaries)
                if digest:
                    weekly_digest = digest
                    asyncio.create_task(
                        set_system_config(digest_key, digest, f"Weekly digest for {topic} week {current_week}")
                    )
            except Exception:
                pass
